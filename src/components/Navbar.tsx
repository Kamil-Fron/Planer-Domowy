              ...
              {isActionMenuOpen && (
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-80 sm:w-92 bg-white rounded-2xl shadow-xl border border-slate-200 py-2 z-50 animate-in fade-in zoom-in-95 duration-100 divide-y divide-slate-100 max-h-[85vh] overflow-y-auto">
                    ...
                    {/* Powiadomienia (Tylko powiadomienia z przejściem do czynności) */}
                    <div className="p-3">
                      ...
                      {/* Kompaktowy przycisk włączenia powiadomień w telefonie, jeśli nie są jeszcze aktywne */}
                      {pushPermission !== 'granted' && isPushSupported() && !isRunningInIframe() && (
                        <div className="mb-2">
                          <button
                            onClick={handleEnablePush}
                            disabled={isSubscribingPush}
                            className="w-full py-1.5 px-2.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold rounded-xl text-[11px] transition-all flex items-center justify-center space-x-1.5 border border-indigo-200/80 cursor-pointer"
                          >
